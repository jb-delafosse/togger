...
class User(db.Model, UserMixin):
    ...
    alias_id = db.Column(
        GUID(), default=uuid.uuid4, nullable=False, unique=True
    )
    ...
    roles = db.relationship(
        "Role", backref="User", cascade="all,delete", lazy=True
    )
    ...
    def generate_validate_token(self):
        return self.auth_v.dumps(
            {
                "username": self.username,
                "valid_until": _gen_valid_until().strftime("%d-%m-%Y"),
            }
        )
    ...
    def check_validate_token(self, token):
        ...
        return datetime.now() < datetime.strptime(
            data["valid_until"], "%d-%m-%Y"
        )

    def generate_password_token(self):
        return self.auth_p.dumps(
            {
                "username": self.username,
                "valid_until": _gen_valid_until().strftime("%d-%m-%Y"),
            }
...
class Role(db.Model):
    ...
    calendar_id = db.Column(
        GUID(), db.ForeignKey("calendar.id"), nullable=False
    )
