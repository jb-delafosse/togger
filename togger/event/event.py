...
def render_report():
    ...
    end = (parser.parse(end_str, dayfirst=True) + timedelta(days=1)).astimezone(
        UTC
    )
    report = event_dao.get_report(start=start, end=end)
    return render_template(
        "report.html", report=report, start=start_str, end=end_str
    )
...
def render_shifts(time_zone=None):
    ...
    if event_id:
        ...
        event = event_dao.generate_event(
            recur_id,
            start=request.args.get("start"),
            end=request.args.get("end"),
        )
    return render_template(
        "shifts_modal.html", event=event, time_zone=time_zone, form=FlaskForm()
    )
