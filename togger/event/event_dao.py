...
def get_events(start, end):
    ...
    events = (
        Event.query.filter(Event.calendar_id == calendar_id)
...
def get_recur_events(start, end, recur_events_unboxed):
    ...
    calendar_id = calendar_dao.get_current_calendar().id
    recur_events = (
        RecurEvent.query.filter(RecurEvent.calendar_id == calendar_id)
        .filter(RecurEvent.start_recur <= end)
        .filter(
            (RecurEvent.end_recur >= start) | (RecurEvent.end_recur.is_(None))
        )
        .all()
    )
    for recur_event in recur_events:
        ...
        if recur_event.end_recur:
            rrule = rrule.replace(
                until=recur_event.end_recur.replace(tzinfo=UTC)
            )
        for start_date in list(
            rrule.between(after=start, before=end, inc=True)
        ):
            ...
            if not any(
                recur_event_unboxed.recur_id == recur_event.id
                and recur_event_unboxed.init_start == start_date
                for recur_event_unboxed in recur_events_unboxed
            ):
                duration = recur_event.end - recur_event.start
                event = generate_event(
                    recur_event.id, start=start_date, end=start_date + duration
                )
                events.append(event)
    return events
...
def save_group_event(
    ...
    if recur_id:
        ...
        rrule_str = get_common_rrule(
            start,
            timezone,
            recur_event.recurrent_type,
            recur_event.recurrent_interval,
        )
        recur_event = RecurEvent(
            ...
            start_recur=start,
            end_recur=original_end_recur,
            all_day=all_day,
            calendar_id=calendar_id,
            rrule=rrule_str,
            recurrent_type=recur_event.recurrent_type,
            recurrent_interval=recur_event.recurrent_interval,
        )

        # update init_date for all related events
        related_events = (
            Event.query.filter(Event.recur_id == recur_id)
        ...
        for event in related_events:
            event.init_start = (
                datetime.combine(
                    event.init_start.replace(tzinfo=UTC)
                    .astimezone(timezone)
                    .date(),
        ...
        rrule_str = get_common_rrule(
            start, timezone, recurrent, recurrent_interval
        )
        recur_event = RecurEvent(
            title=title.strip(),
            description=description,
...
def remove_event(event_id):
    calendar_id = calendar_dao.get_current_calendar().id
    event = (
        Event.query.filter(Event.id == event_id)
...
def get_group_event(recur_id):
    calendar_id = calendar_dao.get_current_calendar().id
    return (
        RecurEvent.query.filter(RecurEvent.id == recur_id)
        .filter(RecurEvent.calendar_id == calendar_id)
        .first()
    )
...
def generate_event(recur_id, start, end):
    ...
    event = Event(
        ...
        description=recur_event.description,
        start=start,
        end=end,
        all_day=recur_event.all_day,
        calendar_id=recur_event.calendar_id,
...
def get_report(start, end, calendar_name="default"):
    if calendar_dao.get_current_calendar():
        ...
        report = (
            db.session.query(
                Shift.person, func.count(Shift.person).label("total")
            )
            .join(Event.shifts)
            .filter(Event.calendar_id == calendar_id)
            .filter(Event.start <= end)
            .filter(Event.end >= start)
            .group_by(Shift.person)
            .all()
        )
        return report
    else:
...
def get_weekday_occurrence(date, timezone):
    ...
    if weekday_occurrence in [1, 2, 3]:
        return weekday_occurrence, "{weekday_occurrence}{date_suffix}".format(
            weekday_occurrence=weekday_occurrence,
            date_suffix=date_suffix[weekday_occurrence],
        )
    else:
        return weekday_occurrence, "{weekday_occurrence}{date_suffix}".format(
            weekday_occurrence=weekday_occurrence, date_suffix=date_suffix[0]
        )


def get_common_rrule(date, timezone, recurrent, recurrent_interval=0):
    rrule_str = "RRULE:INTERVAL={recurrent_interval};".format(
        recurrent_interval=recurrent_interval
    )
    if recurrent == "daily":
        ...
        rrule_str += "FREQ=WEEKLY;WKST=MO;BYDAY={byday}".format(
            byday=weekday[0:2].upper()
        )
    elif recurrent == "monthly":
        weekday = get_weekday(date, timezone)[1]
        rrule_str += (
            "FREQ=MONTHLY;WKST=MO;BYDAY={byday};BYMONTHDAY={bymonthday}".format(
                byday=weekday[0:2].upper(),
                bymonthday=_get_monthly_days(
                    get_weekday_occurrence(date, timezone)[0]
                ),
            )
        )
    elif recurrent == "yearly":
        rrule_str += "FREQ=YEARLY;WKST=MO;BYMONTH={bymonth};BYMONTHDAY={bymonthday}".format(
            bymonth=date.astimezone(timezone).month,
            bymonthday=date.astimezone(timezone).day,
        )
    ...
    dtstart = "DTSTART;TZID={timezone}:{iso_date}".format(
        ...
        iso_date=date.astimezone(timezone).replace(tzinfo=None).isoformat(),
    )
